...
import { getStatusVariant, translateInvoiceDocumentType, translateStatus } from '@/lib/display-labels';
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>POS reserva estas secuencias al facturar dentro del tenant.</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {(sequencesQuery.data ?? []).map((sequence) => (
                <TableRow key={sequence.id}>
                  ...
                  <TableCell>{sequence.endNumber}</TableCell>
                  <TableCell>{sequence.validUntil ? formatDate(sequence.validUntil) : '-'}</TableCell>
                  <TableCell>
                    <Badge variant={getStatusVariant(sequence.status)}>{translateStatus(sequence.status)}</Badge>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
